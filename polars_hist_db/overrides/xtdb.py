...
_OVERRIDE_TIMESTAMP_MIGRATION = "override-timestamps-tz-v1"
_MIGRATIONS_TABLE = "__polars_hist_db_migrations"


def migrate_xtdb_override_timestamps(
    connection: Any, config: OverrideLedgerConfig
) -> int:
    """Atomically rewrite legacy string timestamps as native UTC instants."""

    table_name = _qualified_table_name(config.schema, config.table)
    migrations_table = _qualified_table_name(config.schema, _MIGRATIONS_TABLE)
    if _migration_applied(connection, migrations_table):
        return 0
    if not _override_table_has_utf8_timestamps(connection, config):
        return 0

    table = build_override_table_config(config)
    columns = ", ".join(
        _xtdb_column_identifier(column.name) for column in table.columns
    )
    rows = list(
        connection.execute(
            text(
                f"SELECT {columns}, _valid_from AS migration_valid_from, "
                "_valid_to AS migration_valid_to, "
                "_system_from AS migration_system_from "
                f"FROM {table_name} FOR ALL VALID_TIME FOR ALL SYSTEM_TIME"
            )
        ).mappings()
    )
    operation_ids = [str(row["operation_id"]) for row in rows]
    if len(operation_ids) != len(set(operation_ids)):
        raise ValueError("override timestamp migration found mutable operation rows")

    now = datetime.now(timezone.utc)
    _ensure_migrations_table(connection, migrations_table, now)
    statements = [
        f"ASSERT NOT EXISTS (SELECT 1 FROM {migrations_table} WHERE _id = "
        f"{_literal(_OVERRIDE_TIMESTAMP_MIGRATION, 'TEXT')}), "
        "'override timestamp migration already applied'"
    ]
    if operation_ids:
        ids = ", ".join(
            _literal(operation_id, "TEXT") for operation_id in operation_ids
        )
        statements.append(f"ERASE FROM {table_name} WHERE _id IN ({ids})")
    for source in rows:
        row = {column.name: source.get(column.name) for column in table.columns}
        row[config.valid_from_column] = _utc_timestamp(
            source[config.valid_from_column], config.valid_from_column
        )
        row[config.valid_to_column] = _optional_utc_timestamp(
            source.get(config.valid_to_column), config.valid_to_column
        )
        row["recorded_at"] = _utc_timestamp(
            source.get("recorded_at") or source["migration_system_from"],
            "recorded_at",
        )
        statements.append(
            _insert_statement(
                ...
                valid_from=_utc_timestamp(
                    source["migration_valid_from"], "_valid_from"
                ),
                valid_to=_optional_utc_timestamp(
                    source.get("migration_valid_to"), "_valid_to"
                ),
            )
        )
    statements.append(
        f"INSERT INTO {migrations_table} (_id, applied_at, row_count) VALUES ("
        f"{_literal(_OVERRIDE_TIMESTAMP_MIGRATION, 'TEXT')}, "
        f"{_literal(now, 'DATETIME(6)')}, {len(rows)}::BIGINT)"
    )
    _execute_xtdb_transaction(connection, statements)
    return len(rows)


def _migration_applied(connection: Any, migrations_table: str) -> bool:
    try:
        return (
            connection.execute(
                text(
                    f"SELECT 1 FROM {migrations_table} WHERE _id = "
                    f"{_literal(_OVERRIDE_TIMESTAMP_MIGRATION, 'TEXT')}"
                )
            ).first()
            is not None
...
def _override_table_has_utf8_timestamps(
    connection: Any, config: OverrideLedgerConfig
) -> bool:
    rows = connection.execute(
        text(
            ...
            "AND column_name IN (:valid_from, :valid_to, 'recorded_at')"
        ),
        {
            "schema": config.schema,
            "table": config.table,
            "valid_from": config.valid_from_column,
            "valid_to": config.valid_to_column,
        },
    )
    return any(":utf8" in str(row[0]) for row in rows)


def _ensure_migrations_table(
    connection: Any, migrations_table: str, now: datetime
) -> None:
    try:
        connection.execute(text(f"SELECT 1 FROM {migrations_table} LIMIT 1"))
    except Exception as exc:
        ...
        _execute_xtdb_transaction(
            connection,
            [
                f"INSERT INTO {migrations_table} (_id, applied_at, row_count) "
                f"VALUES ('__bootstrap__'::TEXT, {_literal(now, 'DATETIME(6)')}, "
                "0::BIGINT)",
                f"ERASE FROM {migrations_table} WHERE _id = '__bootstrap__'::TEXT",
            ],
        )
...
def _optional_utc_timestamp(value: object | None, name: str) -> datetime | None:
    return None if value is None else _utc_timestamp(value, name)
