...
def flatten(df: pl.DataFrame) -> pl.DataFrame:
    ...
    """
    struct_cols = [
        col for col, dtype in zip(df.columns, df.dtypes, strict=True) if type(dtype) is pl.Struct
    ]
    list_cols = [
        col for col, dtype in zip(df.columns, df.dtypes, strict=True) if type(dtype) is pl.List
    ]
